          ...
          <Modal.Body>
            <Row className="m-1">
              ...
              <Col>
                ...
                <Row className='mt-3'>
                  <Button
                    variant={showSupervisors ? 'primary' : 'outline-primary'}
                    onClick={() => setShowSupervisors(!showSupervisors)}>
                    {showSupervisors ? 'Hide' : `Show`} Authorized Supervisors
                  </Button>
                  <Collapse in={showSupervisors}>
                    <ListGroup style={{ maxHeight: "400px", overflowY: "auto", overflowX: "hidden" }}>
                      {
                        !tool.training.demo ? <ListGroup.Item>N/A</ListGroup.Item> :
                          tool.authorized_users.length === 0 ? <ListGroup.Item>None</ListGroup.Item> :
                            tool.authorized_users.filter(u => supervisor.completed_by.map(u => u.id).includes(u.id)).map(user =>
                              <ListGroup.Item>
                                <div>{user.name}</div>
                                <div><FormText>{user.email}</FormText> </div>
                              </ListGroup.Item>
                ...
                {
                  (supervisor && !supervisor.completed) || !authorized ? null :
                    <Row className="mt-3">
