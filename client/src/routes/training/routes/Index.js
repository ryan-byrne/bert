...
export default function Index() {
  ...
  const TrainingTile = ({ training }) => {
    ...
    const progress = q.length > 0 ? Math.floor(100 * q.filter(_ => _.completed).length / q.length) : 0;
    ...
    return (
      <ListGroup.Item
        ...
        variant={
          (training.demo && training.demo_completed) || (!training.demo && training.completed) ? "success" :
          (training.demo && !training.demo_completed) || (progress > 0) ? 'warning' :
          q.length === 0 ? 'light' :
          (pre.length > 0) ? 'danger' : 'info'
        }
      >
        ...
        <Row>
          ...
          <Col xs={6} className="mt-auto mb-auto">
            {
              (training.demo && training.demo_completed) || (!training.demo && training.completed) ? <Badge bg="success">Completed</Badge> :
              q.length === 0 ? <Badge bg="secondary">Coming Soon</Badge> : 
              pre.length > 0 ? <Badge bg="danger">Missing {pre[0].name}</Badge> :
              (training.demo && !training.demo_completed) ? <Badge bg="warning">Missing Demo</Badge> :
