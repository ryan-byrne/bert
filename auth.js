...
const checkAuthentication = async (req, res, next) => {
    // Ignore Callback
    if ( ['/auth/callback','/auth/login'].includes(req._parsedUrl.pathname) ) {}
    ...
    // Session user exists, set access_token
    else oauth2Client.setCredentials(req.session.user.tokens)
    next()
}
...
module.exports = {auth, google, oauth2Client, checkAuthentication}